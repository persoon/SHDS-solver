...
//    private int MAX_AGGREGATED_PRICE  = -1;
    ...
    public RuleSchedulerSolver(String homeAgtName, ArrayList<SchedulingRule> schedulingRules,  double[] bgLoadsKWh) {
        ...
        rules.addAll(schedulingRules.stream().filter(SchedulingRule::isActive).collect(Collectors.toList()));
    ...
    public void model(double[] neighborPower) {
        ...
        int maxNeighborsPwConsumption = scaleAndRoundPower(Utilities.getMax(neighborPower));
        int minNeighborsPwConsumption = scaleAndRoundPower(Utilities.getMin(neighborPower));
        ...
        IntVar objPowerDiff = new IntVar(store, "pwDiff", //0, IntDomain.MaxInt);
                (int) Math.pow(aggregatedPowerBounds.getFirst()  + minNeighborsPwConsumption, 2) * HORIZON,
                (int) Math.pow(aggregatedPowerBounds.getSecond() +  maxNeighborsPwConsumption, 2) * HORIZON);
        ...
        for (SchedulingRule rule : rules) {
            ...
            int mID = cpModelId.get(model);
            //System.out.println(rule.getProperty());
...
//                printPredictiveModels();
    ...
    //but with no neighbor power and using searchSatisfaction instead of searchMostConstrainedStatic()
    @Override
